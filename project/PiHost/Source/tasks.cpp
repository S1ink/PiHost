    ...
    while (*(that->external)) {
        ...
        {   //EXECUTED
            ...
            if (linebuffer != "name,output,write,hr,min,sec,args") {
                that->output << pilib::withTime("Task file is not in the correct format - no tasks started\n");
                return;
            }
            ...
            while (std::getline(reader, linebuffer)) {
                ...
                that->threads.emplace_back(pilib::p_routineThread<void, const char*, const pilib::olstream&>,
                    &swap_ptr,
                    std::move(databuffer.tme),
                    that->clock,
                    funcbuff,
                    databuffer.args.c_str(),
                    std::move(pilib::olstream(databuffer.output.c_str(), mode))
                );
        ...
        while (that->clock < pilib::d_untilNext({ 0, 0, 0 })) {
            if (!*(that->external)) {
                break;
            }
        ...
        if (*(that->external)) {
            std::this_thread::sleep_until(pilib::d_nextTime({ 0, 0, 0 }));
        }
    }
    for (size_t i = 0; i < that->threads.size(); i++) {
        if (that->threads[i].joinable()) {
...
void TaskManager::runCommand(const char* message, const pilib::olstream& logs) {
    pilib::olstream out(logs);
    pilib::StopWatch ptime("Total elapsed time", logs, 0);
    ((out <<= "Pihost internal command runner initialized. (") <= pilib::dateStamp()) < ")\n\n";

    pilib::exec(message, out.open());

    out <= newline <= "Process finished at: " <= pilib::dateStamp() < newline;
    ptime.end();
}
void TaskManager::errorName(const char* message, const pilib::olstream& logs) {
    pilib::olstream out(logs);
    out << pilib::withTime("Task name not found\n");
}