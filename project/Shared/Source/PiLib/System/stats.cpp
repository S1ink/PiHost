...
namespace pilib {
	...
	void CPU::CoreData::parse(std::ifstream& reader) {
#ifdef E_CHECKS == 1
	...
	void CPU::CoreData::parse(std::ifstream& reader, const std::streampos& begin) {
		reader.seekg(begin);
		reader >> this->title;
		for (uint i = 0; i < (uint)States::TOTAL; i++) {
			reader >> this->data[i];
	...
	void CPU::CoreData::fromMain() {
		this->parse(0);
	}
	void CPU::CoreData::fromPos(const std::streampos& begin) {
		this->parse(begin);
	}
	void CPU::CoreData::fromTag(const std::string& tag) {
		if (!tag.compare(0, 3, "cpu")) {
			if (isdigit(tag[3])) {
	...
	void CPU::CoreData::fromNum(uint corenum) {
	...
	CPU::CoreData::CoreData(const CoreData& other) :
		title(other.title), hint(other.hint), data(other.data) {}
	CPU::CoreData::CoreData(CoreData&& other) :
		title(std::move(other.title)), hint(std::move(other.hint)), data(std::move(other.data)) {}
	...
	CPU::CoreData& CPU::CoreData::operator=(CoreData&& other) {
		...
		this->hint = std::move(other.hint);
		this->data = std::move(other.data);
	}

	const std::string& CPU::CoreData::getTitle() {
		return this->title;
	}
