...
namespace pilib {
    ...
    void SigHandle::printIgnore(int signum) {
        sigout << pilib::dateStamp() << " : Signal(" << signum << ") caught: {" << sigmap[signum - 1] << "} -> ignored\n";
    }
    void SigHandle::printExit(int signum) {
        sigout << pilib::dateStamp() << " : Signal(" << signum << ") caught: {" << sigmap[signum - 1] << "} -> calling exit\n";
    }
    ...
    void SigHandle::setLog(const std::_Ios_Openmode modes) {
        sigout.setModes(modes);
    }
    void SigHandle::setLog(const char* file) {
        sigout.setStream(file);
    }
    void SigHandle::setLog(const char* file, const std::_Ios_Openmode modes) {
