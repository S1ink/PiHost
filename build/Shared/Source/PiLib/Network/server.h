...
namespace pilib {
	...
	public:
		...
		protected:
			HttpServer* that;
			pilib::olstream* getStream();
			Version* getVer();

			Formatter() : that(nullptr) {}
			void update(HttpServer* outer);
		public:	//fix visiblity issues with default constructor/add guards for nullptr (that)
			...
			virtual void onRequest(int fd, const char* ip, int readlen, Request* req = nullptr, const char* resource = nullptr);
			virtual void onResponse(int fd, const char* ip, int sent, Response* resp = nullptr, const char* resource = nullptr);
		...
		HttpServer(
			...
			const char* root = pilib::progdir.getDir(),
		...
		HttpServer(
			pilib::olstream&& logger = &std::cout,
			const char* root = pilib::progdir.getDir(),
			std::string(*rmapper)(std::string&&, const std::string&) = HttpHandler::find,
			std::atomic<bool>* control = nullptr,
			Version version = Version::HTTP_1_1,
...
//*****************************************************************************************
	...
	public:
		HttpServer2(
			...
			t_handler(this)
		{
			this->t_formatter = static_cast<const format_t&>(format);
			this->t_formatter.update((HttpServer*)this);
		}
		HttpServer2(
			pilib::olstream&& logger = &std::cout,
			const char* root = pilib::progdir.getDir(),
	...
	void HttpServer2<format_t>::HttpHandler2::respond(const int socket, const char* ip, const int readlen, const std::string& input) {
		...
		headers.add(   //headers that apply to all
			(Segment) {
			"Server", "Custom C++ HTTP Server (Raspberry Pi)"
		}
		);
