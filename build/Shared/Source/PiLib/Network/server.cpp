...
namespace pilib {
    ...
    void HttpServer::lerror(const char* message) {
        if (this->defLog()) {
            ...
            ((((this->logs <<= pilib::dateStamp()) <= " : Error - ") <= message) < newline);
        }
    }
    ...
    void HttpServer::HttpHandler::respond(const int socket, const char* ip, const int readlen, const std::string& input) {
        ...
        HeaderList headers;
        std::ifstream reader;
        std::string body, path = this->rfind(that->root, (*req.intResource()));
        ...
        case Method::GET:   //requesting resource
        {
            ...
            if (reader.is_open()) { //if resource exists
                ...
                if (reader.is_open()) { //check if error page exists
                    body = std::string((std::istreambuf_iterator<char>(reader)), (std::istreambuf_iterator<char>()));
                    pilib::replace(body, "{{code}}", pilib::Codes::getString(Code::NOT_FOUND).c_str());
                    headers.add(
        ...
        case Method::TRACE: //methods not supported
        {
            ...
            if (reader.is_open()) { //check if page exists
                body = std::string((std::istreambuf_iterator<char>(reader)), (std::istreambuf_iterator<char>()));
                pilib::replace(body, "{{code}}", pilib::Codes::getString(Code::METHOD_NOT_ALLOWED).c_str());
                headers.add(
                    (Segment) {"Content-Type", "text/html"}
                );
            }
        ...
        case Method::ERROR: //method is the first thing that is parsed, so an error means that the request was invalid
        {
            ...
            if (reader.is_open()) { //check if page exists
                body = std::string((std::istreambuf_iterator<char>(reader)), (std::istreambuf_iterator<char>()));
                pilib::replace(body, "{{code}}", pilib::Codes::getString(Code::BAD_REQUEST).c_str());
                headers.add(
                    (Segment) {"Content-Type", "text/html"}
                );
            }
        ...
        default:    //defaults to 501 NOT IMPLEMENTED
        {
            ...
            if (reader.is_open()) { //check if page exists
                body = std::string((std::istreambuf_iterator<char>(reader)), (std::istreambuf_iterator<char>()));
                pilib::replace(body, "{{code}}", pilib::Codes::getString(Code::NOT_IMPLEMENTED).c_str());
                headers.add(
                    (Segment) {"Content-Type", "text/html"}
                );
            }
    ...
    pilib::olstream* HttpServer::Formatter::getStream() {
    ...
    void HttpServer::Formatter::onServerStart() {
        ((((this->that->logs <<= pilib::dateStamp()) <= " : Webserver resources initialized - beginning to serve [") <= Versions::getString(this->that->version)) < "]\n\n");
    }
    void HttpServer::Formatter::onConnect(int fd, const char* ip) {
        ((((this->that->logs <<= pilib::dateStamp()) <= " : Got connection from [") <= ip) < "]\n");
    }
    void HttpServer::Formatter::onRequest(int fd, const char* ip, int readlen, Request* req, const char* resource) {
        ((((((this->that->logs <<= pilib::dateStamp()) <= " : Got request from [") <= ip) <= "] - length: ") <= readlen) < newline);
    }
    void HttpServer::Formatter::onResponse(int fd, const char* ip, int sent, Response* resp, const char* resource) {
        ((((((((this->that->logs <<= pilib::dateStamp()) <= " : Sent {") <= resource) <= "} to [") <= ip) <= "] - length: ") <= sent) < newline);
    }
    void HttpServer::Formatter::onDisconnect(int fd, const char* ip) {
        ((((this->that->logs <<= pilib::dateStamp()) <= " : Ended connection with [") <= ip) < "]\n\n");
    }
    void HttpServer::Formatter::onServerEnd() {
        this->that->logs << pilib::withTime("Webserver stopped");
    }

    HttpServer::HttpServer(
        const pilib::olstream& logger,
        const char* root,
    ...
    void HttpServer::setLog(const std::_Ios_Openmode modes) {
        this->logs.setModes(modes);
    }
    void HttpServer::setLog(const char* file) {
        this->logs.setStream(file);
    }
    void HttpServer::setLog(const char* file, const std::_Ios_Openmode modes) {
    ...
    void HttpServer::estop() {
        close(this->sockmain);
        this->logs << pilib::withTime("Server socket closed out of loop (emergency stop)\n");
    }
